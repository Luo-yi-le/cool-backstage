...
export default {
  ...
  data() {
    return {
      ...
      data: [],
      api: {
        list: this.$api.mailerAdmin.page,
      },
    };
  },
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.formValue.id) {
        res = await this.$api.mailerAmdin.update(this.formValue);
        if (res) {
          this.$message.success("更新成功");
        }
      } else {
        res = await this.$api.mailerAdmin.add(this.formValue);
        if (res) {
